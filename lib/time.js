...
var reviewer = function(approve) {
  ...
  req({
    ...
    var par = {}
    _.each(workers, function(employee, index) {
      ...
      par[employee] = function(callback) {
        ...
        var wait = Math.round(index * 5000);
        setTimeout(function() {
          printer("Working on employee: "+employee);
          requestEmployeeTime(employee, callback, approve);
        }, wait);
      }
    });
    printer("Requesting time for employees, spacing calls to one every 5 seconds so we don't bring down SPIDAMin");
    async.parallel(par,
      ...
      function(errors, results) {
        ...
        if (approve) {
          printer("All time is now approved.")
        }else{
